-- Prevent Square payment reconciliation from republishing administratively suspended providers.
-- Initial payments may publish only from approved_pending_payment. Renewals may
-- reconcile while published. Suspended providers keep their suspension even if a
-- valid payment arrives.
...
begin
  ...
  if target_billing_mode not in ('one_time','auto_renew') then
    raise exception 'Billing mode must be one_time or auto_renew';
  end if;
  if target_period_end<=target_period_start then
    raise exception 'Billing period end must be after start';
  end if;

  select pp.status,pp.plan
    into publication_status,publication_plan
  ...
  if publication_status not in ('approved_pending_payment','published','suspended') then
    raise exception 'Provider publication status is not eligible for billing activation';
  end if;

  if publication_plan<>target_plan then
    raise exception 'Payment plan does not match provider publication plan';
  end if;
  ...
  if publication_status='approved_pending_payment' then
  ...
  -- If publication_status='suspended', deliberately do not alter publication state.

  insert into public.provider_billing_events(
    provider_id,event_type,external_event_id,amount_cents,currency,details
  )
  values(
    target_provider_id,'square_payment_activated',target_external_event_id,target_amount_cents,'USD',
    jsonb_build_object(
      'plan',target_plan,
      ...
      'publication_status_before_payment',publication_status
    )
  )
...
revoke all on function public.billing_activate_provider(
  uuid,text,text,timestamptz,timestamptz,text,text,text,text,text,integer
) from public,anon,authenticated;

grant execute on function public.billing_activate_provider(
  uuid,text,text,timestamptz,timestamptz,text,text,text,text,text,integer
) to service_role;
